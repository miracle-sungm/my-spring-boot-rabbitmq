package com.miracle.topicproducer.config;
...
public class RabbitConfig {
    ...
    public TopicExchange topicExchange() {
        return new TopicExchange(ExchangeEnum.TOPIC_MESSAGE.getName(), ExchangeEnum.TOPIC_MESSAGE.getDurable()
                , ExchangeEnum.TOPIC_MESSAGE.getAutoDelete());
    }
    ...
    public Binding smsBinding() {
        ...
        return BindingBuilder.bind(smsQueue()).to(topicExchange()).with(RoutingKeyConstant.TOPIC_SMS);
    }

    @Bean
    public Binding emailBinding() {
        //direct模式下需要指定routingKey
        return BindingBuilder.bind(emailQueue()).to(topicExchange()).with(RoutingKeyConstant.TOPIC_EMAIL);
    }
}
